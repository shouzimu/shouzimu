...
public class TreeNodeIterator {
    ...
    /**
     * 按层遍历，根->第二层->第三层 广度优先遍历 <p> 使用队列的先进先出的特点来依次将 父节点->左孩子->右孩子的顺序将节点放入队列中，当队列为空时说明遍历结束 <p> 如果要实现打印时每一层占一行怎么实现呢？
     */
    ...
    /**
     * 深度优先搜索的栈实现，递归本质上就是使用栈来实现的 这个地方我们手动入栈和出栈
     */
